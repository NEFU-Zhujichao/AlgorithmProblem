...
- 对象头
  ...
  - Klass Point（类型指针）
- 实例数据
- 对其填充     Tip：由于虚拟机要求对象起始地址必须是8字节的整数倍，一个空对象占8个字节 
...
### Volatile关键字的作用和实现
- 保证共享数据可见性   
  - 总线嗅探机制：容易产生总线风暴。不断的从主内存嗅探和cas不断循环，无效的交互会导致总线带宽达到峰值。
- 不保证原子性  
...
- 使用原子类，解决原子性问题  AtomicInteger 底层 AQS 貌似是private volatile int state;
### volatile 和 synchronized区别
- volatile只能修饰实例变量和类变量，而synchronized可以修饰方法，以及代码块。
- volatile保证共享数据可见性，但是不保证原子性。而synchronized时一种互斥的机制。
